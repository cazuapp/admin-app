...
class ProductUpdateBloc extends Bloc<ProductUpdateEvent, ProductUpdateState> {
  ...
  ProductUpdateBloc({required this.instance}) : super(const ProductUpdateState.initial()) {
  ...
  Future<void> _onPreload(ManagerPreload event, Emitter<ProductUpdateState> emit) async {
    final id = event.id;
    final key = event.key;
  ...
  void _onOK(AddressInfoEventOK event, Emitter<ProductUpdateState> emit) {
    emit(const ProductUpdateState.initial());
  }

  void _cleanErr(CleanErr event, Emitter<ProductUpdateState> emit) {
    emit(state.copyWith(errmsg: "", status: FormzSubmissionStatus.initial));
  }

  void _onKeyChanged(UpdateBlocKeyChanged event, Emitter<ProductUpdateState> emit) {
    final key = state.key;
  ...
  Future<void> _onSubmitted(UpdateSubmitted event, Emitter<ProductUpdateState> emit) async {
    if (state.isValid) {
      ...
      try {
        ...
        int? result = await instance.products.update(
            id: state.id,
            key: state.key,
