...
class AddressBloc extends Bloc<AddressEvent, AddressState> {
  ...
  AddressBloc({required this.instance}) : super(const AddressState()) {
    on<AddressList>(_onAddressList, transformer: throttleDroppable(throttleDuration));
    on<UserSet>(_onUserSet);
  }

  Future<void> _onUserSet(UserSet event, Emitter<AddressState> emit) async {
    final id = event.id;
  ...
  Future<void> _onAddressList(
    ...
    try {
      ...
      final posts = await _fetchItems(emit: emit, startIndex: state.addresses.length);
      posts.isEmpty
  ...
  Future<List<Address>> _fetchItems({required Emitter<AddressState> emit, int startIndex = 0}) async {
    DualResult? response = await instance.address.get(over: state.userid, offset: startIndex, limit: _postLimit);
