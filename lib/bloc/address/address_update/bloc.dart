...
class AddressUpdateBloc extends Bloc<AddressInfoEvent, AddressInfoState> {
  ...
  AddressUpdateBloc({required this.instance}) : super(const AddressInfoState.initial()) {
    ...
    on<SetData>(_onPreload);
  }

  Future<void> _onAddressProgress(AddressProgress event, Emitter<AddressInfoState> emit) async {
    final value = event.value;

    emit(state.copyWith(value: value, isValid: false, status: FormzSubmissionStatus.initial));
  }
  ...
  Future<void> _onPreload(SetData event, Emitter<AddressInfoState> emit) async {
    final key = event.key;
    final value = event.value;
    ...
    emit(state.copyWith(user: user, id: id, title: title, iconsrc: icon, key: key, value: value, isValid: false));
  }

  void _onKeyChanged(AddressUpdateBlocKeyChanged event, Emitter<AddressInfoState> emit) {
    final key = state.key;
    dynamic value;
  ...
  Future<void> _onSubmitted(AddresssInfoSubmitted event, Emitter<AddressInfoState> emit) async {
    if (state.isValid) {
      ...
      try {
        ...
        int? result =
            await instance.address.update(user: state.user, id: state.id, key: state.key, value: state.finalvalue);
