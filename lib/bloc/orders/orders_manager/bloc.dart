...
class OrderManagerBloc extends Bloc<OrderManagerEvent, OrderManagerState> {
  ...
  Future<void> _onInfoRequest(OrderInfoRequest event, Emitter<OrderManagerState> emit) async {
    final id = event.id;
    final userid = instance.getuser.id;
    ...
    try {
      DualResult? result = await instance.orders.info(id: id, userid: userid);
  ...
  Future<void> _onCancelRequest(OrderCancelRequest event, Emitter<OrderManagerState> emit) async {
    ...
    try {
      int? result = await instance.orders.cancel(userid: userid, id: id);
      ...
      if (result == Protocol.ok) {
        emit(state.copyWith(status: result, current: OrderStatus.cancelsuccess));
      } else {
        emit(state.copyWith(status: result, errmsg: errmsg, current: OrderStatus.failure));
      }
    } catch (_) {
      emit(state.copyWith(status: Protocol.unknownError, current: "Unknown error"));
    }
  }
}
