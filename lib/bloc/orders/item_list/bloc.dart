...
class OrderItemsBloc extends Bloc<OrderItemEvent, OrderItemsState> {
  ...
  Future<void> _onItemsFetched(
    ...
    try {
      ...
      final items = await _fetchItems(emit: emit, startIndex: state.items.length);
  ...
  Future<List<Item>> _fetchItems({required Emitter<OrderItemsState> emit, int startIndex = 0}) async {
    DualResult? status = await instance.orders.getItems(id: state.id, offset: startIndex, limit: _postLimit);
