...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
  ...
  AuthenticationBloc({required this.instance}) : super(const AuthenticationState.unknown()) {
    on<AuthenticationStatusChanged>(_onAuthenticationStatusChanged);

    on<AuthenticationLogoutRequested>(_onAuthenticationLogoutRequested);
    _authenticationStatusSubscription =
        instance.authenticationRepository!.status.listen((status) => add(AuthenticationStatusChanged(status)));
  }
  ...
  Future<void> _onAuthenticationStatusChanged(
      AuthenticationStatusChanged event, Emitter<AuthenticationState> emit) async {
    switch (event.status) {
