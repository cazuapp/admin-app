...
import 'package:cazuapp_admin/validators/email.dart';
import 'package:cazuapp_admin/validators/password.dart';
...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  void _onRememberChanged(LoginRememberChanged event, Emitter<LoginState> emit) {
  ...
  void _onPasswordChanged(LoginPasswordChanged event, Emitter<LoginState> emit) {
    ...
    emit(state.copyWith(password: password, isValid: Formz.validate([password, state.email])));
  }
  ...
  Future<void> _onSubmitted(LoginSubmitted event, Emitter<LoginState> emit) async {
    if (state.isValid) {
      ...
      try {
        DualResult? result = await instance.auth
            .login(email: state.email.value, password: state.password.value, remember: state.remember, doSetup: true);
        String? errmsg = "";

        if (result?.status == Protocol.ok) {
          emit(state.copyWith(result: Protocol.ok, status: FormzSubmissionStatus.success));
        } else {
          switch (result?.status) {
            ...
            case Protocol.noFlags:
              errmsg = "No flags.";
              break;

            case Protocol.noAdmin:
              errmsg = "No flags";
              break;
