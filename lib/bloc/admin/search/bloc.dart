...
class AdminSearchBloc extends Bloc<AdminSearchEvent, AdminSearchState> {
  AppInstance instance;

  AdminSearchBloc({required this.instance}) : super(const AdminSearchState()) {
  ...
  Future<void> _onSearchReset(
    ...
    emit(
      state.copyWith(
        total: 0,
        text: "",
        status: AdminSearchStatus.success,
        users: <User>[],
        hasReachedMax: false,
      ),
    );
  ...
  Future<void> _onSearchRequest(SearchRequest event, Emitter<AdminSearchState> emit) async {
    ...
    try {
      if (state.status == AdminSearchStatus.initial) {
        return emit(state.copyWith(status: AdminSearchStatus.success));
      }

      final posts = await _fetchResults(emit: emit, startIndex: state.users.length);

      posts.isEmpty
          ? emit(state.copyWith(hasReachedMax: true, status: AdminSearchStatus.success))
          : emit(
              state.copyWith(
                status: AdminSearchStatus.success,
      ...
      emit(state.copyWith(status: AdminSearchStatus.failure));
    }
  }
  ...
  Future<List<User>> _fetchResults({required Emitter<AdminSearchState> emit, int startIndex = 0}) async {
    DualResult? response = await instance.users.adminSearch(value: state.text, offset: startIndex, limit: _postLimit);
