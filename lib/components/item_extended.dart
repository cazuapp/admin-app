...
class ItemExtended extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (external) {
      display = const Icon(Icons.keyboard_double_arrow_right_rounded, color: AppTheme.black);
    } else {
      display = const Icon(Icons.navigate_next_sharp, color: AppTheme.rightarrow);
    }
            ...
            child: Stack(children: <Widget>[
              GestureDetector(
                ...
                leading: fawesome != null
                    ? Icon(fawesome, color: red ? AppTheme.white : AppTheme.black, size: 22)
                    : Icon(iconsrc, color: AppTheme.black, size: 22),
                title: Text(
                  input,
                  style: GoogleFonts.ubuntu(
                      fontSize: 16.0, color: red ? AppTheme.white : AppTheme.black, fontWeight: FontWeight.w500),
                ),
              )),
            ])));
  }
