...
class OrderBloc extends Bloc<OrderEvent, OrderState> {
  ...
  Future<void> _onSet(
    ...
    List<String> items = event.request;

    emit(const OrderState());
    emit(state.copyWith(current: items));
  }
  ...
  Future<void> _onOrderFetched(
    ...
    try {
      if (state.status == OrderStatus.initial) {
        ...
        final posts = await _fetchItems(state.current);
        return emit(
          state.copyWith(
      ...
      final posts = await _fetchItems(state.current, state.orders.length);
  ...
  Future<List<OrderList>> _fetchItems([List<String>? conditions, int startIndex = 0]) async {
    DualResult? status = await instance.orders.get(conditions: conditions, offset: startIndex, limit: _postLimit);
