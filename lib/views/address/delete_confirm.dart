...
class DeleteAddressConfirmPage extends StatelessWidget {
  ...
  final int user;
  final String name;
  const DeleteAddressConfirmPage({required this.user, required this.id, required this.name, super.key});
    ...
    return Scaffold(
      body: BlocProvider(
        create: (_) => AddressManagerBloc(instance: BlocProvider.of<AuthenticationBloc>(context).instance)
          ..add(SetBase(user: user, name: name, id: id)),
...
class CloseAddressForm extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String name = context.read<AddressManagerBloc>().state.name;
        ...
        listener: (context, state) {
          if (state.current == AddressStatus.failure) {
            ...
            if (state.current == AddressStatus.success) {
              BlocProvider.of<AddressManagerBloc>(context).add(const AddressManagerOK());
              Navigator.pop(context);
              Navigator.pop(context);
              Navigator.pop(context);
              navigate(context, AddressListPage(id: state.user));
            }
          }
        },
        child: Scaffold(
      ...
      builder: (context, state) {
        return Directionality(
            ...
            child: ElevatedButton.icon(
                ...
                onPressed: () {
                  context.read<AddressManagerBloc>().add(AddressDelete(id: state.id, user: state.user));
                }));
      },
    );
  }
