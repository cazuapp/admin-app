...
import 'package:cazuapp_admin/bloc/user_menu/search/bloc.dart';
...
class FinalUserSearchBar extends StatefulWidget {
  const FinalUserSearchBar({super.key});

  @override
  State<FinalUserSearchBar> createState() => _SearchBarState();
}

class _SearchBarState extends State<FinalUserSearchBar> with SingleTickerProviderStateMixin {
    ...
    return FadeTransition(
      ...
      child: IconButton(
        ...
        onPressed: () {
          ...
          BlocProvider.of<UserSearchBloc>(context).add(SearchReset());
        },
      ),
    );
  }
  ...
  void _handleSearchTextChange(BuildContext context, String text) {
    BlocProvider.of<UserSearchBloc>(context).add(SearchReset());
    if (text.isNotEmpty) {
      BlocProvider.of<UserSearchBloc>(context).add(SearchRequest(text: text));
    }
  }
}
