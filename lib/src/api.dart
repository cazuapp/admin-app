...
/* production queries are sent using base64 encript */
    ...
    if (result?.getStatus() != Httpr.ok) {
      ...
      if (result.geterror() == Protocol.noSecret || result.geterror() == Protocol.invalidSecret) {
    ...
    if (result?.getStatus() == Httpr.invalidToken) {
      ...
      DualResult? dual = await instance.auth
          .login(email: loginEmail.toString(), password: loginPassword.toString(), remember: remember, doSetup: false);
...
class Query {
  ...
  Future<QueryResult?> run() async {
    ...
    if (dotenv.env['env'].toString() == "production") {
      final String hashed = BCrypt.hashpw(dotenv.env['key'].toString(), BCrypt.gensalt(logRounds: 10));
    ...
    try {
      ...
      if (error.type == DioExceptionType.connectionError ||
        ...
        response = QueryResult(status: Httpr.internetFailed, errors: [Protocol.noInternet]);
      } else {
        response = QueryResult.fromJson(error.response?.statusCode, error.response?.data);
      }
    }
